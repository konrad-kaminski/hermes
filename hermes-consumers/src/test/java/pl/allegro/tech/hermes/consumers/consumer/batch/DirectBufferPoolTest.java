...
public class DirectBufferPoolTest {
    ...
    public void testSimple() throws Exception {
        ...
        int size = 1024;
        DirectBufferPool pool = new DirectBufferPool(totalMemory, size, false);
    ...
    public void testNonblockingMode() throws Exception {
        DirectBufferPool pool = new DirectBufferPool(2, 1, false);
        pool.allocate(1);
        try {
            pool.allocate(2);
    ...
    public void testDelayedAllocation() throws Exception {
        DirectBufferPool pool = new DirectBufferPool(5 * 1024, 1024, true);
        ByteBuffer buffer = pool.allocate(1024);
            ...
            try {
                for (int i = 0; i < iterations; i++) {
                    ...
                    if (random.nextBoolean())
                        ...
                        size = random.nextInt((int) pool.totalMemory());
                    ByteBuffer buffer = pool.allocate(size);
                    pool.deallocate(buffer);
                }
                success.set(true);
